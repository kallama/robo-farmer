...
const polygonScanUrl = config.polygon.polygonScanUrl;
...
const validStrategies = ['HOLD', 'SELL', 'COMP', 'HYPCOMP'];
const slippage = Number(process.env.SLIPPAGE);
const sleep = Number(process.env.SLEEP);
const rpcUrl = process.env.CUSTOM_RPC !== '' ? String(process.env.CUSTOM_RPC) : config.polygon.rpcUrl;
